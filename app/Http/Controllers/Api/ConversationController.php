...
class ConversationController extends Controller
{
   /**
    ...
    {
        Log::info('🔍 findAdminByEmail called with email: ' . $request->email);
        ...
        try {
            // Đầu tiên kiểm tra user có tồn tại không
            $user = \App\Models\User::where('email', $request->email)->first();
            Log::info('👤 User check: ' . ($user ? 'Found user: ' . $user->name : 'User not found'));
            
            if ($user) {
                Log::info('🔧 User status: ' . ($user->status ?? 'no status'));
                Log::info('🔧 User role_id: ' . ($user->role_id ?? 'no role_id'));
                
                // Kiểm tra role
                if ($user->role) {
                    Log::info('👑 User role name: ' . $user->role->name);
                } else {
            ...
            Log::info('🎯 Admin query result: ' . ($admin ? 'Admin found: ' . $admin->name : 'Admin not found'));
            ...
            return response()->json([
                'message' => 'Tìm thấy admin thành công',
                'admin' => $admin
            ], 200);

        } catch (\Exception $e) {
            Log::error('❌ Error in findAdminByEmail: ' . $e->getMessage());
            return response()->json([
