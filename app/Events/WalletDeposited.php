...
class WalletDeposited implements ShouldBroadcast
{
     ...
     */
    public function __construct(User $user, float $amount, string $transactionId = null, string $target)
    {
        Log::info('[WalletDeposited] Event được khởi tạo', [
            'user_id' => $user->id,
            'amount' => $amount,
        ...
        $this->notificationData = [
            ...
            'transaction_id' => $transactionId,
            'deposited_at' => now()->format('d/m/Y H:i')
        ];
        
        Log::info('[WalletDeposited] Notification data prepared', $this->notificationData);
        ...
        Log::info('[WalletDeposited] Event constructor completed');
    }
    ...
    {
        $message = "Bạn đã nạp thành công " . number_format($this->amount, 0, ',', '.') . "đ vào ví";
    ...
    {
        $message = "Khách hàng {$this->user->name} đã nạp " . number_format($this->amount, 0, ',', '.') . "đ vào ví";
        ...
        foreach ($adminUsers as $admin) {
            Notification::create([
                'user_id' => $admin->id,
                'type' => 'wallet_deposit_admin',
                'title' => 'Yêu cầu nạp tiền ví',
                'message' => $message,
    ...
    public function broadcastOn(): array
    {
        ...
        Log::info('[WalletDeposited] Broadcasting on channels', [
    ...
    {
        Log::info('[WalletDeposited] Broadcasting with data', $this->notificationData);
