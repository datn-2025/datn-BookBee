            ...
            <li>
                <a href="{{ route('admin.chat.show', $conversation->id) }}"
                    ...
                    <div class="flex-shrink-0 ms-2">
                        ...
                        @if ($conversation->messages->where('sender_id', '!=', auth('admin')->id())->where('read_at', null)->count() > 0)
                            <span class="badge bg-danger rounded-pill ms-1">
                                {{ $conversation->messages->where('sender_id', '!=', auth('admin')->id())->where('read_at', null)->count() }}
                            </span>
                        @endif
                    </div>
                </a>
